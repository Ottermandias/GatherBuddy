...
internal class FishTab
{
    ...
    public          Game.Fish[]                 RelevantFish = Array.Empty<Game.Fish>();
    public          Fish[]                      CachedFish   = Array.Empty<Fish>();
    ...
    public Fish[] SortFish()
    {
        var fish = RelevantFish.Select(f => AllCachedFish[f]);
        return GatherBuddy.Config.FishSortOder switch
        {
            FishSortOrder.EndTime                => fish.OrderBy(f => f.Base.NextUptime(_weather), _uptimeComparer).ToArray(),
            FishSortOrder.Name                   => fish.OrderBy(f => f.NameLower).ToArray(),
            FishSortOrder.BaitName               => fish.OrderBy(f => f.FirstBaitLower).ToArray(),
            FishSortOrder.FishingSpotName        => fish.OrderBy(f => f.FishingSpotLower).ToArray(),
            FishSortOrder.ZoneName               => fish.OrderBy(f => f.TerritoryLower).ToArray(),
            FishSortOrder.Uptime                 => fish.OrderBy(f => f.Uptime).ToArray(),
            FishSortOrder.InverseEndTime         => fish.OrderByDescending(f => f.Base.NextUptime(_weather), _uptimeComparer).ToArray(),
            FishSortOrder.InverseName            => fish.OrderByDescending(f => f.NameLower).ToArray(),
            FishSortOrder.InverseBaitName        => fish.OrderByDescending(f => f.FirstBaitLower).ToArray(),
            FishSortOrder.InverseFishingSpotName => fish.OrderByDescending(f => f.FishingSpotLower).ToArray(),
            FishSortOrder.InverseZoneName        => fish.OrderByDescending(f => f.TerritoryLower).ToArray(),
            FishSortOrder.InverseUptime          => fish.OrderByDescending(f => f.Uptime).ToArray(),
