...
public partial class FishRecorder : IDisposable
{
    ...
    {
        if (record.Catch == null || !record.Flags.HasFlag(FishRecord.Effects.Valid) || record.Flags.HasLure() && !record.Flags.HasValidLure()) //If the fish is caught less than 200ms after leaving the dead zone, don't count it.... Not 100% rigourous, but from testing I saw ~100ms. Doubled it just to be safe. Ping dependant...
            return;
